...
import WeatherBackground from './Componenets/WeatherBackground';
import { WindIcon, HumidityIcon, VisibilityIcon, SunriseIcon, SunsetIcon } from './Componenets/Icons';
import { convertTemperature, getHumidityValue, getVisibilityValue, getWindDirection } from './Componenets/Helper';
...
const App = () => {
  ...
  const API_KEY = 'f4c04a7963b1daafb269597e32f76e58';
 ...
 //https://api.openweathermap.org/data/2.5/weather?lat=${s.lat}&lon=${s.lon}&appid={API_KEY}&units=metric
  ...
  // THIS WOULD FETCH DATA FROM URL AND SET THE WEATHER STATE
        ...
        shadow-2xl p-8 max-w-md w-full border border-white/30 relative z-10  text-white'>
          ...
          {!weather ? (
            <form onSubmit={handleSearch} className='flex flex-col relative'> 
              ...
              className='mb-4 p-3 rounded border border-white bg-transparent text-white outline-none placeholder-white focus:outline-none
              focus:border-blue-300 transition duration-300'/>
              {error && (
                <p className='text-red-300 text-sm mb-2'>{error}</p>
              )}
              {suggestion.length > 0 && (
                <div className='absolute top-12 left-0 right-0 bg-slate-900/90 shadow-md rounded z-10'>
                  {suggestion.map((s) => (
              ...
              <button type='submit' className='bg-indigo-950 hover:bg-blue-700 text-white font-semibold py-2 px-4
            ...
            <div className='mt-6 text-center transition-opacity duration-500'>
              ...
              <div className='flex justify-between items-center'>
                ...
                <button onClick={() => setUnit(u => u==='C' ? 'F' : 'C')}
                  className='bg-blue-700 hover:bg-blue-800 text-white font-semibold py-1
              ...
              <img src={`https://openweathermap.org/img/wn/${weather.weather[0]?.icon}@2x.png`} 
               ...
               <p className='text-4xl text-amber-950'>
               ...
               <div className='mt-6 text-sm'>
                  <p><strong>Feels Like:</strong> {convertTemperature(weather.main.feels_like, unit)}
                  &deg;{unit}</p>
