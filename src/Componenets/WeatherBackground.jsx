        ...
        <div className='fixed inset-0 z-0 overflow:hidden'> 
            {background == video ? (
                <video autoPlay loop muted className='w-full h-full object-cover
                 opacity-100 pointer-events-none animate-fade-in'>
                    <source src={video} type='video/mp4'/>
                </video>
            ) : (
                <img src={background} alt='Weather-bg' className='w-full h-full object-cover
                 opacity-20 pointer-events-none animate-fade-in' />
            )}

            <div className='absolute insert-0 bg-black/30' />
        </div>
    );
}
